...
import BetterArray from 'better-array';
...
@Component({
  ...
  template: `
    ...
    >
      <cell
        *ngFor="let cd of cellDatas; let i = index"
        [cellData]="cd"
        ...
        [row]="
          !setByColumn &&
          i != applyClipIndices.first() &&
          i != applyClipIndices.last() &&
          applyClipIndices.contains(i)
        "
...
export class MatrixComponent {
  ...
  @Input() set displayRowDatas(datas: RowData[]) {
    this.cellDatas = [];
    datas.forEach(
      (data) => (this.cellDatas = this.cellDatas.concat(data.cellDatas))
    );

    this.calculatedWidth =
      datas[0].cellDatas.length * 32 + datas[0].cellDatas.length * 4;
  ...
  setRowDatasAnimated(inputRowDatas: RowData[], setByColumn: boolean) {
    ...
    this.cellDatasToSet = [];
    inputRowDatas.forEach(
      (data) =>
        (this.cellDatasToSet = this.cellDatasToSet.concat(data.cellDatas))
    );
  ...
  calcRowColumnIndexToSet() {
    if (this.setByColumn) {
      ...
      do {
        for (
          let col = this.columnIndexToSet;
          col < this.rowDatasToSet[0].cellDatas.length;
          col++
        ) {
        ...
        if (!isDiff) this.matrixIsSet = true;
      } while (!isDiff && !this.matrixIsSet);
    } else {
      let isDiff = false;
      do {
        for (
          let row = this.rowIndexToSet;
          row < this.rowDatasToSet.length;
          row++
        ) {
          for (
            let col = 0;
    ...
    const cellIndexToSet =
      this.rowIndexToSet * this.cellsPerRow + this.columnIndexToSet;
  ...
  cellIsSxithRow(i: number) {
    const row = Math.floor(i / this.cellsPerRow);
    if (row % 5 == 0 && i > this.cellsPerRow) return true;
  }

  cellIsFithColumm(i: number) {
    const row = Math.floor(i / this.cellsPerRow);
    if ((i + 1) % 5 == 0 && i < (row + 1) * this.cellsPerRow - 1) return true;
  }

  cellIsSixthColumm(i: number) {
    const row = Math.floor(i / this.cellsPerRow);
    if (i % 5 == 0 && i > row * this.cellsPerRow) return true;
  }
